...
DATA_DIR = BASE_DIR / "数据集及类型_备份"
FIELD_DESC_DIR = BASE_DIR / "字段说明"
DATA_FALLBACK = BASE_DIR / "数据集及类型_已处理"
...
def ensure_student_id(df: pd.DataFrame, tname: str, field_mapping: dict) -> pd.DataFrame:
    """任务1：解析与对齐，将学号列统一为 student_id。"""
    col = field_mapping.get(tname) or STUDENT_ID_COLS.get(tname)
    if col and col in df.columns and col != "student_id":
        df = df.rename(columns={col: "student_id"})
    else:
        for c in ["XH", "XSBH", "LOGIN_NAME", "CREATER_LOGIN_NAME", "cardld", "IDSERTAL", "USERNUM", "KS_XH", "SID", "XHHGH"]:
            if c in df.columns and c != "student_id":
...
def apply_winsorization(df: pd.DataFrame) -> Tuple[pd.DataFrame, dict]:
    ...
    stats_dict = {}
    for col in df.select_dtypes(include=[np.number]).columns:
        if col == "student_id" or df[col].dtype == "object":
            continue
        if is_score_column(col):
            method = "zscore"
